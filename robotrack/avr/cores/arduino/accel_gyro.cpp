...
/*   // Send the data to the serial port
  ...
  switch (axis){
        case 'x': {
            if (mode == 2) return (angle_x);
            else if (mode == 1) return (accel_angle_x);
            else if (mode == 0) return (unfiltered_gyro_angle_x);
        }

        case 'y': {
            if (mode == 2) return (angle_y);
            else if (mode == 1) return (accel_angle_y);
            else if (mode == 0) return (unfiltered_gyro_angle_y);
        }
        case 'z': {
            if (mode == 2) return (angle_z);
            else if (mode == 1) return (accel_angle_z);
            else if (mode == 0) return (unfiltered_gyro_angle_z);
        }
